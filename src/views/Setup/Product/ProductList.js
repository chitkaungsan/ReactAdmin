...
const columns= [
  {
    ...
    name: 'Retail Price',
    selector: row => row.retailer_price_with_tax,
    sortable: true,
    grow: 2
  },
  {
    name: 'Retail Price(No Tax)',
    selector: row => row.retailer_price_no_tax,
    sortable: true,
    grow: 2
  },
  {
    name: 'Customer Price',
    selector: row => row.customer_price,
    sortable: true,
    grow: 2
  }, {
         name:"Action",
...
const fetchData = async () =>{
  await axios.get(`${process.env.REACT_APP_API_URL}/api/get/product`,config)
  .then(({data})=>{
...
const _handleSync = () => {
    setBtnLoad(false);
     axios.get(`${process.env.REACT_APP_API_URL}/api/get/fetch/product`,config)
  .then(({data})=>{
      ...
      <CCol xs={12}>
        <CCard className="mb-4">
          ...
          <div className="d-flex justify-content-between">
              <strong className='text-center'>Agency List</strong>
              {btnLoad ?
